...
class ConvertedTableLoader(TableLoader, ABC):
    ...
    def convert_df(self, indf):
        ...
        Args:
            indf (pandas DataFrame or dict of pandas DataFrames): Basically, anything returned by the read_from_file
        ...
        print(f"AFTER CONDENSE: {outdf}")
        # Merge sheets
        outdf = self.merge_df_sheets(outdf)
        print(f"AFTER MERGE: {outdf}")
    ...
    def check_output_dataframe(self, outdf, indf):
        ...
        if len(missing) > 0:
            raise AggregatedErrors().buffer_error(
                RequiredHeadersError(self.revert_headers(missing, indf))
            )
    ...
    def revert_headers(self, headers, in_df):
        ...
        Args:
            ...
            in_df (pandas.DataFrame or Dict[pandas.DataFrame])
        Exceptions:
            None
        Returns:
