...
class PeakGroupLabel(HierCachedModel):
    ...
    def enrichment_fraction(self):
        ...
        try:
            ...
            if warning:
                warnings.warn(
                    f"Unable to compute enrichment_fraction for {self.peak_group.sample}:{self}, {msg}."
                )
    ...
    def normalized_labeling(self):
        ...
        try:
            ...
            warnings.warn(
                f"Unable to compute normalized_labelings for {self.peak_group.sample}:{self}, associated "
    ...
    def animal(self):
        ...
        return self.peak_group.sample.animal
    ...
    def from_serum_sample(self):
        ...
        """
        if self.peak_group.sample.is_serum_sample is None:
            warnings.warn(
                f"Sample {self.peak_group.sample.name}'s is_serum_sample field hasn't been set."
            )
            fss = self.peak_group.sample._is_serum_sample()
        else:
            fss = self.peak_group.sample.is_serum_sample
...
class NoCommonLabel(Exception):
    def __init__(self, peakgrouplabel):
        msg = (
            ...
            f"[{peakgrouplabel.peak_group.sample.animal.infusate.name}])."
        )
        super().__init__(msg)
