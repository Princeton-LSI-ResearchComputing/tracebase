...
def resolve_field_path(
    ...
    if isinstance(field_or_expression, str):
        ...
        for fld in field_or_expression.get_source_expressions():
            if fld is None or (isinstance(fld, str) and fld == ""):
                ...
                elif any(f != "" for f in tmp_field_reps):  # Assumes list
                    field_reps.extend([f for f in tmp_field_reps if f != ""])
    ...
    if field_path is None:
        ...
        if "ordering" in model._meta.__dict__:
            for obf_exp in model._meta.ordering:
                ...
                distinct_fields.extend(_get_distinct_fields_helper(model, str(obf)))
        else:
            distinct_fields.append("pk")
        return distinct_fields
    return _get_distinct_fields_helper(model, str(field_path))
