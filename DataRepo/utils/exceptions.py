...
class RequiredValueErrors(Exception):
    ...
    Errors like this usually only happen when related data failed to load (due to some other error) and is evidenced by
    the fact that the indicated columns/rows have values.  Fixing errors that appear above this will fix this error.
...
class NewResearchers(Exception):
    """Summarization of all NewResearcher exceptions.

    Example:
        > New researchers encountered.  Please check the existing researchers:
        >     George
        >     Frank
        > to ensure that the following researchers (parsed from the indicated file locations) are not variants of
        > existing names:
...
class ConflictingValueError(InfileError, SummarizableError):
    ...
    Instance Attributes:
        ...
        rec_dict (Optional[dict]): Dict created from file representing what was attempted to be loaded.  Optional if the
            exception message was provided.
...
class MissingC12ParentPeak(InfileError, SummarizableError):
    ...
    - The C12 PARENT peak exists, but was not picked in El-Maven.  In this case, the best solution is to redo the peak
      annotation file starting from re-picked peaks from El-Maven that include the parent peak.  Alternatively, the peak
      annotation file could be edited to remove all of that compound's peaks and a subsequent file could be loaded using
      the complete peak group.
...
class MzxmlSequenceUnknown(InfileError, SummarizableError):
    ...
    - Add rows to the Peak Annotation Details sheet that account for all the files (adding 'skip' to the Skip column for
      any files that should be ignored).  This is the preferred solution.
...
class MzxmlNotColocatedWithAnnot(InfileError):
    ...
    mzXML files are assigned an MS Run Sequence based on either the value in the `Sequence` column in the `Peak
    Annotation Details` sheet or (if that's empty), the `Default Sequence` defined in the `Peak Annotation Files` sheet
    and the `Peak Annotation File Name` column in the `Peak Annotation Details` sheet.

    If the default is used, this exception is a warning.  However, if there is no default and no `Sequence` in the `Peak
    Annotation Details` sheet's `Sequence` column, the association is inferred by the directory structure.  By
...
class UnmatchedMzXML(InfileError, SummarizableError):
    ...
    This can occur if there are mzXML files from samples that have not been used to produce a peak annotation file yet,
    i.e. totally unanalyzed samples.  It can also occur if an existing peak annotation file was overlooked when
    generating the Study Doc on the Upload Start page.

    The resolution depends on what you want to do.  If you want to ignore these mzXML files that are unanalyzed, and put
    off their load until a subsequent submission or a later addendum to this submission, every unaccounted-for
    ...
    If you want to load these mzXML files, the best way to proceed is to generate a new template on the Upload **Start**
    page and copy your work from the old study doc to the new one, being careful to note differences such as newly
    inserted ordered sample rows in the Samples sheet.

    There are a few reasons for this strategy.  One of the big ones is that only the Start page checks for Peak Group
    Conflicts.  The other big one is that multiple linked sheets are consistently populated and all inter-sheet
    references are maintained, which is laborious and error prone when attempted manually.
    """
...
class ReplacingPeakGroupRepresentation(InfileError):
    ...
    Group records.  By selecting the new peak annotation file as the peak group representation to load in the `Peak
    Group Conflicts` sheet, this warning informs you that an old previously loaded peak group will be deleted.
...
class DuplicatePeakGroupResolutions(InfileError):
    ...
    ^ _The **same** samples_ means **all** samples.  There is assumed to be no partial overlap between sample sets for_
    _the same compounds because the automated construction of this file separates them programmatically, so be careful_
    _editing the `Peak Group Conflicts` sheet, to make sure you do not introduce partial sample overlap between rows._
    """
...
class MissingDataAdded(InfileError):
    ...
    Examples:
    - A novel animal treatment is entered into the `Treatment` column of the `Animals` sheet in the Study Doc, but not
      into the `Treatments` sheet.  The TraceBase Upload **Validate** page will autofill the new treatment name in a new
      row added to the `Treatments` sheet of the Study Doc.
    - A novel tissue is entered into the `Tissue` column of the `Samples` sheet in the Study Doc, but not into the
      `Tissues` sheet.  The TraceBase Upload **Validate** page will autofill the new tissue name in a new row added to
      the `Tissues` sheet of the Study Doc.
...
class UnknownPeakAnnotationFileFormat(InfileError):
    ...
    To resolve this issue, select the format code using the dropdown menus in the `File Format` column of the `Peak
    Annotation Files` sheet in the Study Doc that corresponds to the reported file.  If none of the supported formats in
    the dropdown match the file format, reach out to the TraceBase team to request adding support for the new format.
    In the meantime, it is recommended that you use one of the supported natural abundance correction tools to
