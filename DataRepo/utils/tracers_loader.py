...
from DataRepo.utils.exceptions import InfileError, CompoundDoesNotExist, RequiredColumnValue
from DataRepo.utils.table_loader import TableLoader
from DataRepo.utils.infusate_name_parser import IsotopeData, TracerData, parse_tracer_string
...
class TracersLoader(TableLoader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Returns:
            Nothing
        """
        self.positions_delimiter = kwargs.pop("positions_delimiter", self.POSITIONS_DELIMITER)
    ...
    def load_data(self):
        """Loads the Tracer and TracerLabel tables from the dataframe.
    ...
    def parse_label_positions(self, positions_str):
        """Create a list of integers from a comma-delimited string of integers.

        Args:
            positions_str (string): comma-delimited string of integers
    ...
    def check_data_is_consistent(
        ...
        compound_str,
        tracer_str,
        inconsistent_compounds,
        inconsistent_names,
        tracer_name_to_number,
        inconsistent_numbers,
    ):
        # Make sure that each tracer number is always associated with the same compound
        if tracer_dict[tracer_number]["compound_name"] != compound_str:
            inconsistent_compounds[tracer_number][
                tracer_dict[tracer_number]["compound_name"]
            ] = [tracer_dict[tracer_number]["rownum"]]
            inconsistent_compounds[tracer_number][compound_str].append(self.rownum)

        # Make sure that each tracer number is always associated with the same tracer name
        if tracer_dict[tracer_number]["tracer_name"] != tracer_str:
            inconsistent_names[tracer_number][
                tracer_dict[tracer_number]["tracer_name"]
            ] = [tracer_dict[tracer_number]["rownum"]]
            inconsistent_names[tracer_number][tracer_str].append(self.rownum)

        if (
            tracer_str in tracer_name_to_number.keys()
            and tracer_number not in tracer_name_to_number[tracer_str].keys()
        ):
            inconsistent_numbers[tracer_str][tracer_number].append(self.rownum)
    ...
    def required_columns_missing(
        ...
        if (
            ...
            missing_col_str = f"{self.headers.NAME} or {', '.join(missing_cols)}"

            self.aggregated_errors_object.buffer_error(
                RequiredColumnValue(
                    column=missing_col_str,
                    rownum=self.rownum,
                    sheet=self.sheet,
                    file=self.file,
                ),
