...
class ExportsOrganizer:
    ...
    def organize_exports_by_study_and_datatype(self, files):
        """Takes a set of export files that includes old and new exports (e.g. files generated by
        StudiesExporter.export()) and it returns 2 dicts containing metadata
        ...
        for file in sorted(files):
            ...
            if study_key and data_type != self.alldatatypes_str:
                ...
                datatype_then_study_data[host][data_type][study_key].append(
                    {
                        "date": date_str,
                        "file": file,
                        "id": study_id,
    ...
    def zip_export_combos(
        ...
        # Original Exports           Filename: {hostname}-{datestamp}-{study_name}-{study_id}-{data_type}.{extension}

        # Zip Packaged Exports:
        # All studies and datatypes  Filename: {hostname}-{datestamp}-allstudies-alldatatypes.zip
        # All studies by datatype    Filename: {hostname}-{datestamp}-allstudies-{data_type}.zip
        # All datatypes by study     Filename: {hostname}-{datestamp}-{study_name}-{study_id}-alldatatypes.zip
...
class ExportParseError(Exception):
    def __init__(self, filename: str):
        super().__init__(f"Unable to parse export file: '{filename}'.")
