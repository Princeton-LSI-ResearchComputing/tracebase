...
from DataRepo.formats.dataformat import ConditionallyRequiredArgumentError
...
class ExportsOrganizer(ExportBase):
    ...
    Class Attributes:
        ExportsOrganizer (this class):
            allstudies_str (str): A string included in the zip archive names indicating that all studies are included in
                the file.
            alldatatypes_str (str): A string included in the zip archive names indicating that all data types are
    ...
    def organize(self, export_dir: str, staging_mode=False, overwrite=False):
        ...
        Args:
            ...
            staging_mode (bool) [False]: Write outputs as .staged files and publish them (and staged files produced by
                StudiesExporter.export()) by renaming them to their final names when processing completes.
            overwrite (bool) [False]: Overwrite all existing packaged zip archives (NOTE: Not export files).
        Exceptions:
            None
        Returns:
            None
    ...
    def organize_exports_by_study_and_datatype(self, files):
        ...
        Exceptions:
            None
        Returns:
            export_data_by_study (Dict[str, Dict[str, Dict[str, List[Dict[str, str]]]]])
        """
        export_data_by_study: Dict[str, Dict[str, Dict[str, List[Dict[str, str]]]]] = (
        ...
        for file in sorted(files):
            ...
            if study_key and data_type != self.alldatatypes_str:
                ...
                if host == self.default_host:
                    study_name = self.study_names[study_id]
                else:
                    # No way to retrieve the actual study name for files from an external host, so we use the slugged
                    # name from the file as a fallback.
                    study_name = slugged_study_name

                export_data_by_study[host][study_key][data_type].append(
                    {
                        "date": date_str,
                        "file": file,
                        "id": study_id,
    ...
    def compute_study_package_filename(
        ...
        Args:
            ...
            study_slug (Optional[str]): The slugified study name as parsed from one of the export files (in case the
                source of the export was an external site and that study is not in the DB).  Required if host does not
        ...
        if study_slug is None and host != self.default_host:
            raise ConditionallyRequiredArgumentError(
                "The 'study_slug' argument is required when the host from the export filename does not match the "
                f"default_host: ({host} != {self.default_host})."
            )
        if host != self.default_host:
            slugified_study_name = str(study_slug)
        else:
        ...
        for host, study_dict in study_packages.items():
            for study_id, package_dict in study_dict.items():
                for package_date, datatype_dict in package_dict.items():
                    study_slug = next(iter(datatype_dict.values()))["slug"]
                    ...
                    exists = self.package_exists(study_package_file)
                    target_file = study_package_file.removesuffix(self.staged_ext)
                    if self.overwrite or not exists:
                        status = "OVERWRITING" if exists else "WRITING"
        ...
        for host, datatype_dict in datatype_packages.items():
            for data_type, package_dict in datatype_dict.items():
                for package_date, study_dict in package_dict.items():
                    ...
                    exists = self.package_exists(datatype_package_file)
                    target_file = datatype_package_file.removesuffix(self.staged_ext)
                    if self.overwrite or not exists:
                        status = "OVERWRITING" if exists else "WRITING"
                        print(f"{status}: {target_file}")
                        study_files = [
    ...
    def zip_everything_packages(
        ...
        for host, package_dict in all_packages.items():
            for package_date, study_dict in package_dict.items():
                ...
                exists = self.package_exists(all_package_file)
                target_file = all_package_file.removesuffix(self.staged_ext)
                if self.overwrite or not exists:
                    status = "OVERWRITING" if exists else "WRITING"
                    print(f"{status}: {target_file}")
                    all_files = [
