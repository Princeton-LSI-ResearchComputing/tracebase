...
class BSTQueryView:
    ...
    Instance Attributes:
        ...
        interface_initialized (bool): Whether init_interface has been called.  Used in get_queryset to validate
            readiness.
        ...
        if isinstance(col, BSTManyRelatedColumn):
            ...
            try:
                ...
                raise AttributeError(
                    f"Annotation '{col.name}' is not present on the record.  Are you sure you passed in a record "
                    f"generated by {cls.__name__}?  Use get_queryset to obtain records."
                )
...
class InterfaceNotInitialized(Exception):
    def __init__(self):
        message = (
            "get_queryset called before initializing the interface.  Call init_interface() in order to apply required "
            "annotations, apply the default sort, and apply the filters to the record objects in the QuerySet."
        )
