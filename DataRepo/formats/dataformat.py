...
class Format:
    ...
    def getFieldUnitsLookup(self):
        ...
        string, and convert function.  For example, for a key of `sample__animal__age`, a lookup of that
        key would look like:

        returned_dict["sample__animal__age"] -> {
            ...
            "months": {
                "name": "months",
                "example": "1.0",
    ...
    def getTrueJoinPrefetchPathsAndQrys(self, qry):
        """
        Takes a qry object and a units lookup dict (that maps the path version of fld [e.g. sample__animal__age] to a
        dict that contains the units options, including most importantly, a convert function that is found via the
